...
to go
  ...
  ask turtles [
    ...
    if (state = "qif") and (controlled? = false)[
      let prob random-float 1
      if prob < H [
        get-i
      ]
    ...
    if (state = "qef") and (controlled? = false)[
      let prob random-float 1
      if prob < H [
        get-e
      ]
    ...
    if my-neighbor != nobody [
      ;remembering the interactions (for tracking strategy)
      if tracking-memory != 0 [
          ifelse length previous-neighbors < tracking-memory [ set previous-neighbors lput [who] of my-neighbor previous-neighbors ]
        [
          set previous-neighbors but-first previous-neighbors
          set previous-neighbors lput [who] of my-neighbor previous-neighbors
        ]
      ]
      ;set previous-neighbor [who] of my-neighbor
...
;to-report num-susceptibles
;  report count turtles with [(state = "f") or (state = "s")]
;end

;to-report num-quarantined
;  report count turtles with [(state = "qef") or (state = "qif") or (state = "qf")]
;end

to-report cumulative-infected
  report count turtles with [got-sick? = true]
end

to-report num-controlled
  report count turtles with [(controlled? = true) and (state != "recovered")]
end
...
to test-and-track
  ask n-of control-size turtles with [(state = "s") or (state = "e") or (state = "ef") or (state = "f") or (state = "i") or (state = "if")] [
    if ((state = "e") or (state = "ef")) [
      get-qef
      set controlled? true
      foreach previous-neighbors quarantine-previous-neighbor
    ]
    if ((state = "i") or (state = "if")) [
      get-qif
      set controlled? true
      foreach previous-neighbors quarantine-previous-neighbor
    ]
...
to quarantine-previous-neighbor [x]
  ask turtle x [
    if ((state = "e") or (state = "ef")) [
      get-qef
      set controlled? true
...
"quarantined" 1.0 0 -16383231 true "" "plot count turtles with [(state = \"qef\") or (state = \"qif\") or (state = \"qf\")]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot num-controlled"
...
"" ""
PENS
"max-infected" 1.0 0 -5298144 true "" ";plot count turtles with [(state = \"i\") or (state = \"if\") or (state = \"qif\") ]\nplot max-infected"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [(state = \"qef\") or (state = \"qif\") or (state = \"qf\")]"
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <go>go</go>
    <timeLimit steps="70"/>
    <metric>num-susceptibles</metric>
    ...
    <enumeratedValueSet variable="Hi">
      <value value="0.2"/>
    </enumeratedValueSet>
  ...
  <experiment name="intervention-testing" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
    <metric>num-infected</metric>
    <metric>num-controlled</metric>
    <enumeratedValueSet variable="landa-2">
      <value value="0.23"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-percent-i">
    ...
    <enumeratedValueSet variable="control-size">
      <value value="1"/>
      <value value="5"/>
      <value value="10"/>
    </enumeratedValueSet>
  ...
  <experiment name="max-infected" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
    ...
    <enumeratedValueSet variable="tracking-memory">
      <value value="1"/>
      <value value="10"/>
      <value value="40"/>
    </enumeratedValueSet>
