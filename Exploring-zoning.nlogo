...
to find-new-spot [x y]

  let myposition "here"
  if (x >= abs x) and (y >= abs y) [ set myposition "first-q" ]
  if (x < abs x) and (y >= abs y) [ set myposition "second-q" ]
  if (x >= abs x) and (y < abs y) [ set myposition "fourth-q" ]
  if (x < abs x) and (y < abs y) [ set myposition "third-q" ]

  if myposition = "first-q" [
    setxy (random-float 80) (random-float 80)
  ]

  if myposition = "second-q" [
    setxy (random-float -80) (random-float 80)
  ]

  if myposition = "third-q" [
    setxy (random-float -80) (random-float -80)
  ]

  if myposition = "fourth-q" [
    setxy (random-float 80) (random-float -80)
  ]

  if any? other turtles-here [ find-new-spot x y ]
...
<experiments>
  ...
  <experiment name="zoning-experiment-max-infected" repetitions="200" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
  ...
  <experiment name="normal-everytick" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
